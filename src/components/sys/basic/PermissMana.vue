  ...
  <div>
    ...
    <div style="margin-top: 20px; width: 700px">
      <el-collapse v-model="activeName" @change="fnChangeCollapse" accordion>
        <el-collapse-item v-for="(role, index) in roles" :key="index"  :title="role.nameZh" :name="role.id">
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      fnInitRoles() {
        this.getRequest('/system/basic/role/list').then(res => {
          if(res) {
            this.roles = res
          }
        })
      ...
      fnDeleteRole(role) {
        this.$confirm('此操作将会永久删除['+role.nameZh+']角色，是否继续？','提示', {
          ...
          this.deleteRequest('/system/basic/role/delete/'+role.id).then(res => {
            if(res) {
              this.fnInitRoles()
            }
          })
